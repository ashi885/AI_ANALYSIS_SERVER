...
function findMatchingTemplate(
    ...
    return templates.find(t =>
        (t.duration_max === 0 || duration <= t.duration_max) &&
...
export async function getDevModeData(
    ...
    devLogger.info(undefined, `getDevModeData(moduleName=${moduleName}, duration=${duration}, clientId=${clientId})`);
    ...
    for (const searchName of searchNames) {
        ...
        `).all(searchName, clientId) as DevTemplate[];
        ...
        `).all(searchName) as DevTemplate[];

        devLogger.info(undefined, `  searchName=${searchName}: ${clientSpecific.length} client-specific, ${globalTemplates.length} global templates`);
        ...
        if (match) {
            devLogger.info(undefined, `  => matched ${matchSource} template #${match.id} "${match.label}" (module=${match.module_name}, min=${match.duration_min}, max=${match.duration_max})`);
            const parsed = JSON.parse(match.template_data);
            if (typeof parsed === 'string') {
                if (searchName === 'transcription' || searchName === 'whisper') {
                    devLogger.info(undefined, `  => returning textToTranscriptionJson (string data, duration=${duration})`);
                    return textToTranscriptionJson(parsed, duration);
                }
                devLogger.info(undefined, `  => string data but not transcription, breaking to fallback`);
                break;
            }
            if (searchName === 'subtitle_translation' && moduleName.startsWith('subtitle_translation_')) {
                devLogger.info(undefined, `  => generic translation template matched but specific language needed, breaking to fallback`);
                break;
            }
            const result = { ...parsed, requestId: `${searchName}_${Date.now()}` };
            if (searchName === 'subtitle_translation' && !result.isMultiResult) {
                const lang = moduleName.startsWith('subtitle_translation_')
                    ...
                    : 'ko';
                devLogger.info(undefined, `  => returning single-language translation result for ${lang}`);
                return {
                    isMultiResult: true,
                    results: [{
                        moduleName: 'subtitle_translation',
            ...
            devLogger.info(undefined, `  => returning template result: ${devLogger.preview(result)}`);
            return result;
        }
    }

    devLogger.info(undefined, `  => no template match, using fallback for ${moduleName}`);
    return generateFallbackDummy(moduleName, duration);
}

export function calculateDevCost(clientModuleRates: string | null | undefined, moduleName: string): number {
    if (!clientModuleRates) return 0;
    try {
        const rates = typeof clientModuleRates === 'string' ? JSON.parse(clientModuleRates) : clientModuleRates;
        return rates[moduleName]?.cost_per_job || 0;
    } catch {
        return 0;
    }
}

export function textToTranscriptionJson(text: string, duration?: number): any {
...
function generateSingleTranslation(lang: string, duration: number): any {
    ...
    const langLabels: Record<string, string> = { ko: 'KO', zh: 'CN', en: 'EN' };
    const langPrefixes: Record<string, string> = { ko: '[Korean]', zh: '[Chinese]', en: '[English]' };
    const label = langLabels[lang] || lang.toUpperCase();
    const prefix = langPrefixes[lang] || `[${lang.toUpperCase()}]`;
    return {
        content: JSON.stringify({
            language: lang,
            segments: segments.map(s => ({ ...s, text: `${prefix} ${s.text}` })),
...
function generateFallbackDummy(moduleName: string, duration: number): any {
    devLogger.info(undefined, `    generateFallbackDummy(moduleName=${moduleName}, duration=${duration})`);
    if (moduleName.startsWith('subtitle_translation_')) {
        const lang = moduleName.replace('subtitle_translation_', '');
        devLogger.info(undefined, `    → generateSingleTranslation(lang=${lang})`);
        return generateSingleTranslation(lang, duration);
    }
    switch (moduleName) {
        case 'transcription':
        case 'whisper':
            devLogger.info(undefined, `    → generateDummyTranscription`);
            return generateDummyTranscription(duration);
        case 'subtitles':
            devLogger.info(undefined, `    → generateDummySubtitles`);
            return generateDummySubtitles(duration);
        case 'metadata':
            devLogger.info(undefined, `    → generateDummyMetadata`);
            return generateDummyMetadata(duration);
        case 'ad_breaks':
            devLogger.info(undefined, `    → generateDummyAdBreaks`);
            return generateDummyAdBreaks(duration);
        case 'promo_breaks':
            devLogger.info(undefined, `    → generateDummyPromoBreaks`);
            return generateDummyPromoBreaks(duration);
        case 'subtitle_translation':
            devLogger.info(undefined, `    → generateDummyTranslation`);
            return generateDummyTranslation(duration);
        case 'vision_ai':
            devLogger.info(undefined, `    → generateDummyVisionAI`);
            return generateDummyVisionAI();
        default:
            devLogger.info(undefined, `    → generic fallback for ${moduleName}`);
            return { content: JSON.stringify({ note: `Dummy data for ${moduleName}` }), usage: { totalTokens: 10 }, cost: 0, requestId: `dummy_${Date.now()}` };
    }
}

function generateDummySegments(duration: number, count: number): any[] {
...
function generateDummyTranscription(duration: number): any {
...
function generateDummySubtitles(duration: number): any {
    const count = Math.max(2, Math.min(20, Math.floor(duration / 15)));
    const segments = generateDummySegments(duration, count);
    return {
...
function generateDummyMetadata(duration: number): any {
...
function generateDummyAdBreaks(duration: number): any {
...
function generateDummyPromoBreaks(duration: number): any {
    const count = Math.max(1, Math.min(5, Math.floor(duration / 300)));
    const breakSpacing = duration / (count + 1);
    const highlights = [];
    for (let i = 1; i <= count; i++) {
...
function generateDummyTranslation(duration: number): any {
    const count = Math.max(2, Math.min(10, Math.floor(duration / 30)));
    const segments = [];
    for (let i = 0; i < count; i++) {
        const start = (duration / count) * i;
    ...
    const makeResults = (lang: string, prefix: string, label: string) => ({
        moduleName: 'subtitle_translation',
        resultType: `subtitle_${lang}`,
        resultData: {
            language: lang,
...
function generateDummyVisionAI(): any {
    return {
        content: JSON.stringify({
