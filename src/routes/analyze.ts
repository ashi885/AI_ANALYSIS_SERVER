...
// Prompt templates for each module - structured for clarity
...
export function normalizeTranscript(input: unknown): string {
...
// Helper: Build structured prompt parts for a module
interface PromptParts {
    system: string;
    user: string;
}

export function buildPromptParts(moduleName: string, transcript: string, params: Record<string, any> = {}): PromptParts {
    ...
    const template = PROMPT_TEMPLATES[lookupName as keyof typeof PROMPT_TEMPLATES];
    ...
    if (params.examples && Array.isArray(params.examples) && params.examples.length > 0) {
        const examplesText = params.examples.map((ex: any, i: number) => {
...
analyzeRouter.post('/analyze', licenseMiddleware, async (req: LicensedRequest, res: Response) => {
    ...
    try {
        ...
        const moduleName = body.moduleName as string | undefined;
        ...
        const pricing = moduleName ? await getModulePricing(clientId!, moduleName, duration) : null;
        ...
        let userMessageContent: any = userContent;
        if (moduleName === 'vision_ai' || (body.images && Array.isArray(body.images))) {
            const imageContents = (body.images as Array<{ timecode: number; base64: string }>) || [];
            userMessageContent = [
        ...
        if (billingType === 'CREDIT' && moduleCost > 0) {
            const deduction = await deductCredits(clientId!, moduleCost, `Execution of module: ${moduleName || 'custom'}`, body.jobId as string || undefined);
...
analyzeRouter.post('/module/:moduleName', licenseMiddleware, async (req: LicensedRequest, res: Response) => {
    const startTime = Date.now();
    const clientId = req.client?.id;
    ...
    try {
        ...
        const usage = (result.usage as any) || { promptTokens: 0, completionTokens: 0, totalTokens: 0, totalTokensIn: 0, totalTokensOut: 0 };
        const requestId = result.id || `req_${Date.now()}`;
        const jobId = (body.jobId || body.local_job_id || null) as string | number | null;
        const userId = (body.userId || body.user_id || null) as number | null;
        ...
        let lowCreditWarning = null;
        if (billingType === 'CREDIT' && moduleCost > 0) {
            const deduction = await deductCredits(clientId!, moduleCost, `Execution of module: ${moduleName}`, body.jobId as string || undefined);
            if (deduction.success && deduction.balance !== undefined && deduction.balance < 5.0) {
                lowCreditWarning = `Low credit balance: $${deduction.balance.toFixed(2)}. Please top up soon.`;
            }
        }
...
analyzeRouter.post('/module/:moduleName/async', licenseMiddleware, async (req: LicensedRequest, res: Response) => {
    const clientId = req.client?.id;
    const moduleName = req.params.moduleName as string;
    ...
    try {
        ...
        const jobId = await enqueueAIJob(clientId!, moduleName, body);
...
analyzeRouter.get('/job/:jobId', licenseMiddleware, async (req: LicensedRequest, res: Response) => {
