...
exports.partitionTranscript = partitionTranscript;
...
async function processAiJob(jobId, audioPath, modulesRequested, clientId, clientName, durationRequested = null, targetLanguages, abortSignal) {
    ...
    const job = (0, db_mgmt_1.getAiJob)(jobId);
    const userId = job === null || job === void 0 ? void 0 : job.user_id;
    const originalFilename = (job === null || job === void 0 ? void 0 : job.filename) || 'job_result';
    const outputDir = userId ? (0, db_mgmt_1.getUserSetting)(userId, 'output_directory') : null;
    if (outputDir) {
        logger_1.logger.info('AI', 'AUTO_OUTPUT_ENABLED', `Job ${jobId} will output results to ${outputDir}`, { originalFilename });
    }
    // Helper to safely write to output directory using templates
    const saveToOutput = async (moduleName, data, extension = 'json', customSuffix) => {
        ...
        try {
            ...
            const fullPath = path_1.default.join(outputDir, fileName);
            fs_1.default.writeFileSync(fullPath, content);
            logger_1.logger.info('AI', 'AUTO_OUTPUT_SAVED', `Saved ${moduleName} output to ${fullPath} (Template: ${finalExt})`);
        }
        catch (e) {
            logger_1.logger.error('AI', 'AUTO_OUTPUT_FAILED', `Failed to save ${moduleName} output: ${e.message}`);
        }
    };
    try {
        ...
        const whisperApiKey = await (0, db_mgmt_1.getClientApiKey)(clientId, 'openai');
        if (!whisperApiKey)
        ...
        const hasOpenRouter = !!openRouterApiKey && modulesRequested.some((m) => m !== 'transcription');
        if (hasOpenRouter && !openRouterApiKey) {
            throw new Error('OpenRouter API key missing for analysis modules');
        }
        const configuredModels = await (0, license_1.getClientModels)(clientId);
        ...
        if (modulesRequested.includes('transcription') && !existingResultsMap['transcription']) {
            ...
            db.prepare('UPDATE ai_jobs SET result_data = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(JSON.stringify(resultData), jobId);
            logger_1.logger.info('AI', 'JOB_PROGRESS_SYNC', `Synced transcription for Job ${jobId}`);
        }
        ...
        if (!transcriptionResult && modulesRequested.some((m) => m !== 'transcription')) {
            ...
            // Priority 2: Fallback - Check siblings (other jobs for the same asset)
                ...
                logger_1.logger.info('AI', 'TRANSCRIPTION_FALLBACK', `Searching for existing transcription for asset ${localId}`);
                try {
                    ...
                    `).get(localId, jobId);
                    if (siblingJob === null || siblingJob === void 0 ? void 0 : siblingJob.result_data) {
                        const siblingResults = JSON.parse(siblingJob.result_data);
                        const sibTranscription = siblingResults.find((r) => r.moduleName === 'transcription');
                        if (sibTranscription) {
                            transcriptionResult = sibTranscription.resultData;
                            logger_1.logger.info('AI', 'TRANSCRIPTION_RECOVERED', `Found transcription from sibling job for ${localId}`);
                    ...
                    logger_1.logger.error('AI', 'TRANSCRIPTION_FALLBACK_FAILED', `Error looking up sibling transcription: ${e.message}`);
                }
            }
            // Final check
        ...
        if (modulesRequested.includes('subtitles') && !existingResultsMap['subtitles']) {
            ...
            db.prepare('UPDATE ai_jobs SET result_data = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(JSON.stringify(resultData), jobId);
            logger_1.logger.info('AI', 'JOB_PROGRESS_SYNC', `Synced subtitles for Job ${jobId}`);
        }
        ...
        const otherModules = modulesRequested.filter((m) => m !== 'transcription' && m !== 'subtitles');
        if (otherModules.length > 0 && openRouterApiKey) {
            ...
            const modulesToProcess = otherModules.filter((m) => !existingResultsMap[m]);
            if (modulesToProcess.length === 0) {
                logger_1.logger.ai('AI_ALL_MODULES_SKIPPED', `Job ${jobId} all modules already completed`, { clientId, jobId });
            }
            else {
                ...
                const modulePromises = modulesToProcess.map(async (moduleName) => {
                    var _a, _b, _c, _d, _e;
                    let model = (_a = configuredModels.find((m) => m.module_name === moduleName)) === null || _a === void 0 ? void 0 : _a.api_model;
                    if (!model) {
                        model = globalFallback;
                        if (!model) {
                            logger_1.logger.warn('AI', 'NO_MODEL_CONFIGURED', `No model configured for module ${moduleName} and no global fallback set. Skipping module.`, { clientId, moduleName });
                            return null;
                        }
                        logger_1.logger.warn('AI', 'USING_GLOBAL_FALLBACK', `No model configured for module ${moduleName}, defaulting to global setting: ${model}`, { clientId, moduleName });
                    }
                    const duration = durationRequested || (transcriptionResult === null || transcriptionResult === void 0 ? void 0 : transcriptionResult.duration) || 0;
                    const pricing = await (0, db_mgmt_1.getModulePricing)(clientId, moduleName, duration);
                    const billablePrice = (pricing === null || pricing === void 0 ? void 0 : pricing.cost_per_job) || 0;
                    ...
                    if (moduleName === 'subtitle_translation') {
                        ...
                        const rawDetected = ((transcriptionResult === null || transcriptionResult === void 0 ? void 0 : transcriptionResult.language) || 'en').toLowerCase();
                        ...
                        for (const lang of languages) {
                            ...
                            if (existingResultsMap[targetKey] && !existingResultsMap[targetKey].partial) {
                                logger_1.logger.ai('AI_TRANSLATION_SKIPPED', `Job ${jobId} using existing ${lang} translation`, { clientId, jobId });
                            ...
                            const transcriptionModule = resultData.find(r => r.module_name === 'transcription' || r.moduleName === 'transcription');
                            const transcriptSegments = ((_d = transcriptionModule === null || transcriptionModule === void 0 ? void 0 : transcriptionModule.result_data) === null || _d === void 0 ? void 0 : _d.segments) || ((_e = transcriptionModule === null || transcriptionModule === void 0 ? void 0 : transcriptionModule.resultData) === null || _e === void 0 ? void 0 : _e.segments) || (transcriptionResult === null || transcriptionResult === void 0 ? void 0 : transcriptionResult.segments) || [];
                            const existingData = existingResultsMap[targetKey];
                            const existingCount = ((existingData === null || existingData === void 0 ? void 0 : existingData.segments) || []).length;
                            const allChunks = partitionTranscript(transcriptSegments, 600, 15000);
                            let chunksToProcess = allChunks;
                            let allTranslatedSegments = [...((existingData === null || existingData === void 0 ? void 0 : existingData.segments) || [])];
                            let totalCost = 0;
                            let totalTokens = 0;
                            let totalLatency = 0;
                            try {
                                if (existingCount > 0) {
                                    ...
                                    if (completedChunksCount > 0 && completedChunksCount < allChunks.length) {
                                        logger_1.logger.ai('AI_TRANSLATION_RESUMING', `Resuming ${lang} for Job ${jobId} from chunk ${completedChunksCount}`, { clientId, jobId });
                                        chunksToProcess = allChunks.slice(completedChunksCount);
                                    }
                                }
                                logger_1.logger.ai('AI_TRANSLATION_BATCHING', `Translating ${lang} in ${chunksToProcess.length} batches for Job ${jobId}`);
                                for (let i = 0; i < chunksToProcess.length; i++) {
                                    ...
                                    let translatedSegments = (dataActual === null || dataActual === void 0 ? void 0 : dataActual.segments) || (Array.isArray(dataActual) ? dataActual : []);
                                    if (translatedSegments.length === 0 && (dataActual === null || dataActual === void 0 ? void 0 : dataActual.raw)) {
                                        try {
                                            const match = dataActual.raw.match(/\"segments\"\s*:\s*(\[[\s\S]*?\])/);
                                            if (match)
                                                translatedSegments = JSON.parse(match[1]);
                                        }
                                        catch (e) { }
                                    }
                    ...
                    try {
                        ...
                        if (shouldChunk) {
                            ...
                            logger_1.logger.ai('AI_CHUNKED_PROCESS', `Processing ${moduleName} in ${chunks.length} chunks`, { jobId, details: { duration } });
                            if (moduleName === 'ad_breaks' || moduleName === 'promo_breaks') {
                                ...
                                if (chunks.length > 1) {
                                    logger_1.logger.ai('AI_SYNTHESIS_START', `Synthesizing ${allCandidates.length} candidates for ${moduleName}`, { jobId });
                                    const synthesis = await callAI(`${moduleName}_synthesis`, '', {
                                        global_target: parseInt(String((0, db_mgmt_1.getTieredValue)(settings.target_frequency || (moduleName === 'ad_breaks' ? '4' : '6'), duration))),
                                    ...
                                    const targetCount = parseInt(String((0, db_mgmt_1.getTieredValue)(settings.target_frequency, duration) || sensibleDefault));
                            ...
                            const result = await callAI(moduleName, (transcriptionResult === null || transcriptionResult === void 0 ? void 0 : transcriptionResult.text) || '', {
                                target_count: (0, db_mgmt_1.getTieredValue)(settings.target_frequency || '3-5', duration),
                                rating_country: settings.rating_country || 'US',
                                chunk_start: 0,
                        ...
                        logger_1.logger.error('AI', 'MODULE_FAILED', `Module ${moduleName} failed: ${rawError}`, err.stack, { clientId });
        ...
        try {
            ...
            logger_1.logger.error('AI', 'QUEUE_SYNC_ERROR', `Failed to sync queue costs for job ${jobId}`, qErr);
        }
        db.prepare('UPDATE ai_jobs SET status = ?, result_data = ?, total_cost_usd = ?, provider_cost_usd = ?, error_message = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(finalStatus, JSON.stringify(resultData), absoluteTotalBilled, absoluteTotalProvider, errorMessage, jobId);
        const resultTypes = resultData.map(r => r.resultType || r.moduleName);
        logger_1.logger.info('AI', 'JOB_PROCESSOR_DONE', `Job ${jobId} finished with status: ${finalStatus}`, {
            clientId,
            billedTotalCost,
            successfulModules,
            failedModules,
        ...
        if (failedModules.length === 0 && fs_1.default.existsSync(audioPath)) {
            try {
        ...
        logger_1.logger.error('AI', 'JOB_PROCESSOR_ERROR', `Job ${jobId} failed: ${rawMessage}`, error.stack, { clientId });
        // Always sanitize before saving to ai_jobs (client-facing)
        const sanitized = sanitizeAIError(rawMessage);
        db.prepare('UPDATE ai_jobs SET status = ?, error_message = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run('error', sanitized, jobId);
    }
}
// Format helpers
...
function splitLongSegments(segments, maxChars = 50) {
    if (!segments)
        return [];
    const result = [];
    for (const seg of segments) {
        ...
        if (currentText)
            result.push({ start: localStart, end: seg.end, text: currentText });
    }
    return result;
}
 ...
 */
function partitionTranscript(segments, maxWindowSeconds = 900, maxChars = 35000) {
